...
function AgoraVideoChat({ sessionId, userId, onEnd, apiUrl = '/api/v1' }) {
  ...
  const initializeAgoraChat = async (uid) => {
    try {
      ...
      const appId = process.env.REACT_APP_AGORA_APP_ID || process.env.NEXT_PUBLIC_AGORA_APP_ID;
      ...
      logger.info('Published local tracks', { sessionId });

    } catch (error) {
      logger.error('Error initializing Agora chat:', error);
  ...
  const handleUserPublished = async (user, mediaType) => {
    try {
      ...
      logger.error('Error subscribing to remote user:', error);
    }
  };

  const handleUserUnpublished = (user, mediaType) => {
    logger.info('User unpublished', { uid: user.uid, mediaType });
    if (mediaType === 'video') {
  ...
  const handleUserJoined = (user) => {
    logger.info('User joined channel', { uid: user.uid });
    setConnectionStatus('Peer connected');
  };

  const handleUserLeft = (user) => {
    logger.info('User left channel', { uid: user.uid });
    setHasRemoteUser(false);
    setConnectionStatus('Peer disconnected');
  };

  const handleConnectionStateChange = (curState, prevState) => {
    logger.info('Connection state changed', { from: prevState, to: curState });
  ...
  const cleanup = async () => {
    try {
      ...
      logger.info('Cleaned up Agora resources');
    } catch (error) {
  ...
  const toggleCaptions = () => {
    ...
    logger.info('Captions toggled', { enabled: !captionsEnabled });
  };
